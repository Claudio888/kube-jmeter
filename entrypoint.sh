...
[[ -z ${JVM_XMS} ]] && JVM_XMS=$(($freeMem/10*4))
...
echo "Upload result to minio bucket jmter-results"

sleep 20

datefolder=$(date +"%d-%m-%Y-%H-%M-%S")

cd ${JMETER_HOME}

chmod +x ./minioupload.sh

./minioupload.sh $MINIO_ADDRESS $MINIO_ACCESS_KEY $MINIO_SECRET_KEY jmeter-results $datefolder results.jtl
./minioupload.sh $MINIO_ADDRESS $MINIO_ACCESS_KEY $MINIO_SECRET_KEY jmeter-results $datefolder jmeter.log